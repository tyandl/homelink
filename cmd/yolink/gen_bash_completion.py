#!/usr/bin/env python3
"""Generate a bash completion script for the yolink_api CLI from the device definitions.
...
Usage:
  ...
  # then `source completions/yolink_api.bash`, or install under /etc/bash_completion.d/
...
# Subcommands and global flag values are defined by the CLI (cmd/yolink), not the devices.
...
def main() -> None:
    ...
    script = f"""\
#!/usr/bin/env bash
# yolink_api bash completion — generated by cmd/yolink/gen_bash_completion.py. Do not edit by hand;
# regenerate after changing device definitions in pkg/yolink/devices/.

__yolink_api_commands="{' '.join(COMMANDS)}"
__yolink_api_global_flags="{' '.join(GLOBAL_FLAGS)}"
__yolink_api_log_levels="{' '.join(LOG_LEVELS)}"
__yolink_api_output_formats="{' '.join(OUTPUT_FORMATS)}"
__yolink_api_methods="{' '.join(method_tokens)}"

# Param flags for a `do` method token (qualified "Type.method" or bare "method").
__yolink_api_method_params() {{
    case "$1" in
...
_yolink_api() {{
    ...
    case "$prev" in
        --log)
            COMPREPLY=( $(compgen -W "$__yolink_api_log_levels" -- "$cur") )
            return ;;
        --output)
            COMPREPLY=( $(compgen -W "$__yolink_api_output_formats" -- "$cur") )
            return ;;
    ...
    if [[ -z "$cmd" ]]; then
        if [[ "$cur" == -* ]]; then
            COMPREPLY=( $(compgen -W "$__yolink_api_global_flags" -- "$cur") )
        else
            COMPREPLY=( $(compgen -W "$__yolink_api_commands" -- "$cur") )
        fi
        return
    fi
    ...
    case "$cmd" in
        ...
        do)
            ...
            if (( COMP_CWORD == method_idx )); then
                COMPREPLY=( $(compgen -W "$__yolink_api_methods" -- "$cur") )
                return
            fi
            ...
            if [[ "$prev" == --* && "$prev" != "--on" ]]; then
                local choices
                choices="$(__yolink_api_param_values "$method" "$prev")"
                if [[ -n "$choices" ]]; then
            ...
            COMPREPLY=( $(compgen -W "--on $(__yolink_api_method_params "$method")" -- "$cur") )
            ;;
    esac
}}

complete -F _yolink_api yolink_api
"""
